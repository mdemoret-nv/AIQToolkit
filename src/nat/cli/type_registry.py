...
from nat.utils.debugging_utils import is_interactive_mode
...
class TypeRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Functions
        self._registered_functions: dict[str, RegisteredFunctionInfo] = {}
    ...
    def register_function(self, registration: RegisteredFunctionInfo):
        if registration.full_type in self._registered_functions:
            if is_interactive_mode():
                logger.warning(f"A function with the same config type `{registration.full_type}` has already been "
                               "registered. Overriding the previous registration since this is an interactive mode. "
                               "This is not recommended and may cause unexpected behavior. Please ensure that the "
                               "function is only registered once.")
            else:
                raise ValueError(
                    f"A function with the same config type `{registration.full_type}` has already been registered.")

        self._registered_functions[registration.full_type] = registration
